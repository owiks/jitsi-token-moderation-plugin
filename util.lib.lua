...
function Util.new(module)
    ...
    self.requireRoomClaim = module:get_option_boolean('asap_require_room_claim', false);
...
function Util:process_and_verify_token(session)
    ...
    elseif self.asapKeyServer and session.auth_token ~= nil then
        ...
        if self.cachedKeys and self.cachedKeys[self.appId] then
            key = self.cachedKeys[self.appId];
        else
            key = self:get_public_key(self.appId);
        end
